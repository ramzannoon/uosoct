...
class TransferVehicle(models.TransientModel):
    ...
    name = fields.Many2one('odoocms.student', 'Student Name', readonly=True)
    ...
    def vehicle_transfer(self):
        ...
        for rec in self:
            ...
            participants = [prt_id.id for prt_id
                            in vehi_data.vehicle_participants_ids]
            if rec.participation_id.id in participants:
                participants.remove(rec.participation_id.id)
            vehi_data.write({'vehicle_participants_ids': [(6, 0, participants)]})
            # entry of participant in new vehicle.
            participants = [prt_id.id for prt_id
                            in vehi_new_data.vehicle_participants_ids]
            participants.append(rec.participation_id.id)
            vehi_new_data.write({'vehicle_participants_ids': [(6, 0, participants)
                                                           ]})
