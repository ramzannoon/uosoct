...
class FacultyAttendance(http.Controller):
    ...
    def class_attendance_page(self, id=0, date_begin=None, date_end=None, sortby=None, **kw):
            ...
            if len(class_ids) > 0:
                ...
                for term_class in class_ids:
                    ...
                    for att in attendance_roaster:
                        ...
                        roaster.append({'roaster_id':att.id,'date':att.date_class,'status':att.state, 'marked': att.att_marked ,'makeup_class': att.makeup_class, 'time_from':att.time_from,'time_to':att.time_to})
        ...
        if len(class_ids) > 0:
            ...
            for term_class in class_ids:
                ...
                roaster = []
                for att in attendance_roaster:
                    roaster.append({'roaster_id': att.id, 'date': att.date_class, 'status': att.state, 'marked': att.att_marked, 'time_from': att.time_from, 'time_to': att.time_to})
                classes.append({'class_name': term_class.name, 'class_code': term_class.code, 'term_class_id': term_class.id, 'data': roaster})
    ...
    def daily_attendance_sheet_save(self, id=0, date_begin=None, date_end=None, sortby=None, **kw):
        try:
            ...
            if attendance_sheet and attendance_sheet.faculty_id != faculty_staff.id:
                if faculty_staff.id not in attendance_sheet.class_id.faculty_ids.mapped('faculty_staff_id'):
