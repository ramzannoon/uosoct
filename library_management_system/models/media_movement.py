...
class OpMediaMovement(models.Model):
    ...
    media_id = fields.Many2one('op.media', 'Reference', required=True)
    media_unit_id = fields.Many2one(
        'op.media.unit', 'Reference Unit', required=True,
    ...
    library_card_id = fields.Many2one(
        'op.library.card', 'Library Card', required=True, domain=[('type', 'in', ['student', 'faculty'])],
        track_visibility='onchange')
    issued_date = fields.Date('Issued Date', readonly=True, default=fields.Date.today())
    return_date = fields.Date('Due Date', readonly=True,)
    ...
    media_type_id = fields.Many2one(related='media_id.media_type_id',
                                    store=True, string='Reference Type')
    ...
    def issue_media(self):
        ''' function to issue reference '''
        for record in self:
            reference_issuance = ""
            if record.student_id:
                reference_issuance = self.env['op.media.movement'].search_count([('student_id', '=', record.student_id.id),
                                                                           ('state', '=', 'issue')])
            if record.faculty_id:
                reference_issuance = self.env['op.media.movement'].search_count([('faculty_id', '=', record.faculty_id.id),
                                                                           ('state', '=', 'issue')])
            if reference_issuance >= record.library_card_id.library_card_type_id.allow_media:
                raise ValidationError(_(
                    'Maximum Number of Reference allowed Limit is full'))
    ...
    def create_penalty_invoice(self):
        for rec in self:
            account_id = False
            product = self.env.ref('library_management_system.op_product_7')
            if product.id:
